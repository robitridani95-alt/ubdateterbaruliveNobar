...
function setupSheets() {
  ...
  const defaults = {
    ...
    QRIS_URL: '',
    NAMA_BANK: 'BRI',
    NO_REKENING: '000000000000',
    ATAS_NAMA: 'Panitia Live Sport'
  };
...
function kirimDonasi(data) {
  if (!data || !data.nama || !data.nominal || !data.noHp || !data.password) {
    return { success: false, message: 'Nama, nominal, No. HP, dan password wajib diisi.' };
  }
  let buktiUrl = '';
  try {
    if (data.buktiBase64 && data.buktiFileName) {
      buktiUrl = simpanFileKeDrive_(data.buktiBase64, data.buktiFileName, 'Bukti Donasi - Live Sport');
    }
  } catch (err) {
    return { success: false, message: 'Gagal mengunggah bukti transfer: ' + err.message };
  }
  ...
  sheet.getRange(rowIndex, 1, 1, 10).setValues([[
    id, data.nama, data.nominal, String(data.noHp).trim(), String(data.password).trim(), data.metode || 'Transfer/QRIS', buktiUrl, data.pesan || '-', 'Menunggu', nowWIBString_()
  ]]);
  return { success: true, message: 'Terima kasih atas donasi kopinya! Simpan No. HP & password untuk login melihat link live setelah diverifikasi panitia.', id: id };
}
...
// ------------------------------------------------------------
// ADMIN: UPDATE CONFIG (QRIS, Rekening, dll)
// ------------------------------------------------------------
function updateConfig(data) {
  const sheet = getOrCreateSheet_('Config', ['Key', 'Value']);
  const rows = sheetToObjects_(sheet);
  Object.keys(data).forEach(key => {
    if (key === 'qrisBase64' && data[key]) {
      const url = simpanFileKeDrive_(data[key], 'qris.png', 'QRIS Live Sport');
      const target = rows.find(r => r.Key === 'QRIS_URL');
      if (target) sheet.getRange(target._row, 2).setNumberFormat('@').setValue(url);
      else sheet.appendRow(['QRIS_URL', url]);
      return;
    }
...
function upsertLinkCabang(data) {
  ...
  if (target) {
    sheet.getRange(target._row, 2).setNumberFormat('@').setValue(data.linkUrl);
  ...
  sheet.getRange(rowIndex, 1, 1, 4).setValues([[data.cabang, data.linkUrl, data.keterangan || '-', nowWIBString_()]]);
