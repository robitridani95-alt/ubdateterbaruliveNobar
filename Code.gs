...
function setupSheets() {
  ...
  // LogoURL = logo club, otomatis dibuatkan dari inisial nama cabang kalau admin tidak isi sendiri
  getOrCreateSheet_('LinkCabang', ['Cabang', 'LinkURL', 'LogoURL', 'Keterangan', 'Timestamp']);
...
function getAllLinkCabang() {
  const sheet = getOrCreateSheet_('LinkCabang', ['Cabang', 'LinkURL', 'LogoURL', 'Keterangan', 'Timestamp']);
  ensureHeaderColumn_(sheet, 'LogoURL'); // migrasi otomatis untuk sheet lama yang belum punya kolom ini
  return sheetToObjects_(sheet);
}

// Tambah baru jika cabang belum ada, atau update jika sudah ada (upsert by Cabang)
// Logo club otomatis dibuatkan dari inisial nama cabang kalau admin tidak upload logo sendiri,
// jadi penonton selalu melihat logo apa pun yang terjadi.
function upsertLinkCabang(data) {
  ...
  const colLogo = ensureHeaderColumn_(sheet, 'LogoURL');
  ...
  const logoBersih = data.logoUrl ? normalizeUrl_(data.logoUrl) : buatLogoOtomatis_(data.cabang);
  const rows = sheetToObjects_(sheet);
  ...
  if (target) {
    ...
    sheet.getRange(target._row, colLogo).setNumberFormat('@').setValue(logoBersih);
  ...
  sheet.getRange(rowIndex, colLogo).setNumberFormat('@').setValue(logoBersih);
