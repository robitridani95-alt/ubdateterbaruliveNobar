...
function kirimDonasi(data) {
  // Pendaftaran nonton GRATIS - tidak butuh nominal/bukti transfer, tapi tetap
  // wajib diverifikasi admin dulu (status awal "Menunggu") sebelum bisa lihat
  // link streaming. Pendaftar yang SUDAH pernah diverifikasi sebelumnya tidak
  // terpengaruh oleh perubahan ini - datanya tidak diubah/disentuh sama sekali.
  ...
  const rows = sheetToObjects_(sheet);
  const nomorSudahAda = rows.some(r => String(r.NoHP) === String(data.noHp));
  if (nomorSudahAda) {
    return { success: false, message: 'No. HP ini sudah pernah mendaftar. Gunakan No. HP lain atau langsung login.' };
  }
  ...
  sheet.getRange(rowIndex, 1, 1, 10).setValues([[
    id, data.nama, 0, String(data.noHp).trim(), String(data.password).trim(), 'Gratis', '', data.pesan || '-', 'Menunggu', nowWIBString_()
  ]]);
  return { success: true, message: 'Pendaftaran berhasil! Akun kamu perlu diverifikasi panitia dulu sebelum bisa nonton. Login pakai No. HP & password ini setelah statusnya Terverifikasi.', id: id };
}
